package projectEuler.p011_LargestProductInAGrid

val grid =
        listOf(listOf(8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8),
...
fun main() {
    val result = largestProductGrid(grid)
    println(result)
}

fun largestProductGrid(grid: List<List<Int>>) = maxOf(rowCheck(grid), columnCheck(grid), diagonalCheck(grid))

fun rowCheck(grid: List<List<Int>>): Long {
    var max: Long = -1
...
fun columnCheck(grid: List<List<Int>>): Long {
    var max = -1L
    val g = grid.map { row -> row.map { it.toLong() } }
    val cols = ArrayList<List<Long>>()

    repeat(g.first().size) { i ->
        cols.add(sequence { g.forEach { yield(it[i]) } }.toList())
    }
...
fun diagonalCheck(grid: List<List<Int>>) = maxOf(rDiagonal(grid), lDiagonal(grid))

fun rDiagonal(grid: List<List<Int>>): Long {
    var max = -1L
    val g = grid.map { row -> row.map { it.toLong() } }
...
fun lDiagonal(grid: List<List<Int>>): Long {
    var max = -1L
    val g = grid.map { row -> row.map { it.toLong() } }

    for (i in 0 until g.size - 3) {
        for (j in 3 until g.size) {
            val result = g[i][j] * g[i + 1][j - 1] * g[i + 2][j - 2] * g[i + 3][j - 3]
            max = maxOf(max, result)
        }
    }
